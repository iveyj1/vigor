...
**Count prefixes** — digits `1-9` accumulate in `self.count`; subsequent nonzero digits extend it. `0` is always the zero-column motion, so a pending count makes `N0` move to column zero on the Nth line below rather than contributing another count digit. When a motion key arrives, it executes `max(count, 1)` times. Count resets to 0 after any non-digit key.
