...
**BufferState** — bundles a `Buffer` with per-buffer state: cursor position (`cx`, `cy`), logical-line scroll offset, wrapped-row top offset (`wrap_skip`), Markdown presentation state/projection (`md_view`, `md_lines`, `md_maps`, `md_languages`), and undo/redo history (`_undo_stack`, `_redo_stack`, `_undo_save_depth`, `_undo_branched`). Uses `__slots__` for efficiency. Created once per opened file. Opening or creating a buffer replaces an untouched initial unnamed buffer rather than retaining it.
