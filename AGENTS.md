...
- `test_vig.py` — PTY-based smoke tests and focused layout checks (plain asserts, no framework, 362 test functions)
...
**Visual edit ops** — `d`/`x`, `y`, `c`, and linewise `>` / `<` work in both VISUAL and VISUAL_LINE modes. `_visual_delete` and `_visual_yank` normalize the selection via `_selection_range`, then delegate to `_delete_range` / `_yank_range`. After the operation, mode returns to NORMAL (or INSERT for `c`).
