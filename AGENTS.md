...
- `test_vig.py` — PTY-based smoke tests and focused layout checks (plain asserts, no framework, 388 test functions)
...
**Panic recovery backups** — `opt_recovery` defaults off; `opt_recoverydelay` defaults to 1000 milliseconds. When enabled, dirty named buffers are serialized after the idle delay to adjacent files named `.vigor-recover.<basename>` without modifying the edited file or clearing dirty state. Successful explicit writes, autosaves, reloads from disk, and undo transitions back to the saved state explicitly delete that buffer's panic backup; generic dirty callbacks never delete files. Opening a file with an existing panic backup reports its path for manual inspection or `:read`; recovery files are never auto-loaded.
