...
- `test_vig.py` — PTY-based smoke tests and focused layout checks (plain asserts, no framework, 398 test functions)
...
Add simplified Vim-compatible hard wrapping: `:set textwidth=N` (default `0`) supplies the hard-wrap width; `gq{motion}`, `gqq`, text-object forms, and Visual `gq` hard-wrap every touched logical line by inserting real newlines, preferring whitespace before the width and falling back to hard splits for long words. Existing short lines are not joined/reflowed.
