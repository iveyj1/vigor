...
`gf` opens an existing file or switches to its existing buffer without discarding edits. Relative paths resolve from the current working directory, unlike `:vigfiles` entries, which resolve from the list file's directory. Paths are whitespace-delimited or enclosed in single/double quotes (spaces allowed); quotes are delimiters, not shell quoting rules. Both forms expand `~`, `$NAME`, and `${NAME}` from the editor's environment. Undefined variables, directories, and missing files report errors. Shell execution, parameter operators, glob expansion, quote escaping, Markdown-link parsing, and line/column suffixes are not supported. The cursor must be within the path token, not on whitespace or past EOL.
