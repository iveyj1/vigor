...
| `<space>s{char}{label}` | label visible occurrences of `{char}` and jump to the selected label; the first occurrence uses `{char}` as its label; works in Visual and as a `d`/`y`/`c` motion |
