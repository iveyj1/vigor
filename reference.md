...
| `<space>s{char}{label}` | label visible occurrences of `{char}` and jump to the selected label; `{char}` labels the first visible match after the cursor, or the last before it if none follow; works in Visual and as a `d`/`y`/`c` motion |
